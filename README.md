...
<img width="1374" height="1057" alt="image" src="https://github.com/user-attachments/assets/37c2db65-6461-4820-8f55-02f882617b7a" />
...
*   **🎨 現代化 UI 設計**:
    ...
    *   **全域熱鍵支援**: 鎖定畫面只需按 `Enter` 即可一鍵登入。
*   **☁️ 自動化後端更新**: 利用 Cron Triggers 定時從外部數據源拉取 IP 並在 Worker 後端進行可用性測試。
*   **⚡ 瀏覽器端真實測速**: 在管理後台直接進行瀏覽器端的真實延遲測試，結果最貼近用戶當前網路實際體驗。
*   **📡 智能路由系統**: 支援子域名路由，訪問不同子域名自動返回對應數據（無需 Token 即可公開訂閱）：
    *   `fast.xxx` / `fast-xxx`: 返回後端測速篩選的優選 IP
    *   `browser.xxx` / `web.xxx`: 返回瀏覽器端實測的優選 IP
    *   `all.xxx` / `ips.xxx`: 返回完整的 IP 庫
*   **🔐 智慧安全管理**: 
    *   支援密碼登入保護與 Token 權限管理。
    *   **密碼記憶優化**: 登出後自動保留密碼輸入，重登更方便。
*   **🔌 便捷 API 集成**: 提供純淨的 API 連結複製功能，方便一鍵導入 Surge, Clash, Sing-box 等代理工具。
...
*   **瀏覽器測速**: 點擊後會對隨機抽取的 IP 進行本地 Ping 測試。測速完成後，結果會自動同步至雲端 (`browser.xxx` 即可讀取)。
...
*支援參數*: `?format=ip` (僅輸出純 IP，不帶機房名稱與延遲備註)
